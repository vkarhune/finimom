...
#'
#' @examples
multi_finimom <- function(beta, se, eaf, R,
                    n = NULL, k = NULL, omega = NULL,
                    cs = TRUE,
                    cs_level = 0.95,
                    pip = FALSE,
                    maxsize = 10, tau = NULL, r = 1,
                    niter = 62500, burnin = 12500,
                    seed = 456, excl.burnin = TRUE,
  ...
  if(maxsize == 1){
    if(0){ # this not yet implemented for multivariate
    ...
    samples <- posterior_samples_multitrait(
      beta = beta, se = se, eaf = eaf, R = R, k = k, omega = omega,
      maxsize = maxsize, tau0 = tau, r0 = r, p = p, u = u, n = n,
    ...
    num_signals <- sapply(1:k, function(i){
      topmod <- as.numeric(strsplit(names(sort(table(samples[[3]]), decreasing = TRUE)[1]), " ")[[1]])
      sum(topmod > (i - 1)*p_clumped & topmod <= i*p_clumped)
    })

    sets <- get_multitrait_credible_sets(
      input = list(value = samples[[1]], modsize = samples[[2]],
                   modindices = sapply(samples[[3]], function(x) { gsub(" ", ",", x) })),
      num_signals = num_signals, level = 0.95, purity = purity,
