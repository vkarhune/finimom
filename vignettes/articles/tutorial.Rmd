...
remotes::install_packages("vkarhune/finimom")
...
The output is a list, where the first element (`samples`) has information on the Markov Chain Monte Carlo samples, and the second element is a list of the obtained 95\% credible sets.
...
If using LD matrix from a reference panel, set `insampleLD = FALSE`:
